function [timesMean,amplitudes] = runRJMCMC(data,designChoices,parameters,wantingDiagnostics)
...
for j = 1:numSamples % loop over samples
    ...
    if ((u < p_birth) || isempty(A)) && (length(A) < maxN)
        ...
        % do a birth step
        [t_i,A] = birthStep(t_i,A,data,indMax,maxN,parameters);
        if wantingDiagnostics
            if length(A) > N
    ...
    elseif u - p_birth < p_death
        ...
        % do a death step
        [t_i,A] = deathStep(t_i,A,data,indMax,maxN,parameters);
        if wantingDiagnostics
            if length(A) < N
        ...
        [t_i,A] = neutralStep(t_i,A,data,indMax,parameters); % neutralStep_old one gets better results...
    end
